...
function ExcludedPluginsList({ search }: { search: string; }) {
    ...
    const ExcludedReasons: Record<"web" | "discordDesktop" | "vesktop" | "desktop" | "dev", string> = {
        ...
        dev: "Developer version of SudoCord"
    };
...
function PluginSettings() {
    ...
    const [newPlugins] = useAwaiter(() => DataStore.get("SudoCord_existingPlugins").then((cachedPlugins: Record<string, number> | undefined) => {
        ...
        DataStore.set("SudoCord_existingPlugins", existingTimestamps);
    ...
    for (const p of sortedPlugins) {
        ...
        if (isRequired) {
            const tooltipText = p.required || !depMap[p.name]
                ? "This plugin is required for SudoCord to function."
